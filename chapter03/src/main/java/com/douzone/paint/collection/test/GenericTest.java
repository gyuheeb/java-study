 ...
 * 1) hierachy min
 * 2) 부모들만(자신만) 사용가능
 * 3) <T super Type(Shape)> => 
 * 오류 <- 함수 구현 블록 (클래스 구현블록 )안에서 T타입이 모호하기 때문에 구현이 불 가 능
 * Type Erasing의 결과 <T super Type>는 Object이기 때문에 문법적인 의미가 없다.
 * 결론은 존재하지 않는다.
 */
	
//	public static /*<T super Shape>*/ /*<? super Shape>*/ void drawShape(T t) {
//		t.draw();
//	}
	 ...
	 * 
	 * 1) 제네릭 타입의 클래스를 사용할 때 (메소드 등 어디에든 사용 불가), 제한(extends, super와 함께)적으로 
	 * 제너릭 타입을 결정해야 할 때 사용한다.
	 * 2) 사용할 수 없는 예)
	 * 	-이 클래스를 제네릭 타입으로 정의할까?			 
	 * 		 (MyStack<T>안됨)
	 * 	-이 클래스를 Shape 자식의 제네릭 타입으로 정의할까?
	 * 		<(?넣으면 안됨.)T extends Shape>
	 * 	-(ex)이 메소드의 첫 파라미터를 제네릭 타입으로 할까? 
	 * 		<?> void m(? t) (절대 no)
	 * 	- 이 메소드의 첫 피라미터의 Shape 자식의 제네릭 타입으로 할까?	
	 * 		<?> extends Shape m(? t) (절대 no)
	 * 
	 * 
	 * 3) 사용할 수 있는 예)
	 * 	-제네릭 클래스 MyStack의 제네릭 파라미터를 Shape의 자식으로만 하자.
	 * 		MyStack<? extends Shape> O
	 * 	--제네릭 클래스 MyStack의 제네릭 파라미터를 Shape의 부모으로만 하자.
