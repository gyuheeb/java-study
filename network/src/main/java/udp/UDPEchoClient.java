...
public class UDPEchoClient {
	private static final String SERVER_IP ="127.0.0.0";
	public static void main(String[] args) {
		...
		try {
			...
			while(true) {
				...
				//4. 받기
				DatagramPacket rcvPacket = new DatagramPacket(new byte[UDPEchoServer.BUFFER_SIZE],UDPEchoServer.BUFFER_SIZE);  //받는 편지 
				socket.receive(rcvPacket); //blocking
				
				byte[]rcvDate = rcvPacket.getData();
				int offset = rcvPacket.getLength();
				String message = new String(rcvDate,0,offset,"utf-8");
